...
from handset.backend.client import AgentDTO
from handset.widgets import scroll_panel, section_label
...
class AgentsPanel(Gtk.Box):
    def __init__(self, *, on_refresh: Callable) -> None:
        ...
        self.status_lab.add_css_class("handset-muted")
    ...
    def show_error(self, msg: str) -> None:
        ...
        empty.add_css_class("handset-fail")
    ...
    def _row(self, agent: AgentDTO) -> Gtk.Widget:
        ...
        outer.add_css_class("handset-agent-row")

        top = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        dot = Gtk.Box()
        dot.add_css_class("handset-dot")
        status = (agent.status or "unknown").lower()
        if status == "online":
            dot.add_css_class("handset-dot-live")
        elif status == "idle":
            dot.add_css_class("handset-dot-busy")
        elif status == "offline":
            dot.add_css_class("handset-dot-off")
        else:
            dot.add_css_class("handset-dot-idle")
        dot.set_valign(Gtk.Align.CENTER)
        top.append(dot)

        host = Gtk.Label(label=agent.hostname or agent.id or "?", xalign=0)
        host.add_css_class("handset-agent-host")
        host.set_hexpand(True)
        top.append(host)

        st = Gtk.Label(label=status.upper(), xalign=1)
        st.add_css_class(f"handset-status-{status if status in ('online','idle','offline') else 'unknown'}")
        top.append(st)
        outer.append(top)
        ...
        mlab.add_css_class("handset-agent-meta")
        outer.append(mlab)
